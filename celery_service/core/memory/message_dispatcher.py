...
class MessageDispatcher:
    def __init__(self, message_system, agent_registry, data_store,max_workers=5):
        ...
        self.dispatcher_thread = None  # Thread will be initialized in start()
         # Create a logger with a custom component name:
        base_logger = setup_custom_logger("MessageDispatcher")
        self.logger = logging.LoggerAdapter(
            base_logger,
            extra={'chain_id': 'N/A', 'component': 'Dispatcher'}
        )
        self.logger.info("MessageDispatcher started in background")
        self.cached_agents = {}
    ...
    def listen_for_messages(self):
        ...
        while not self.stop_event.is_set():
            ...
            if message:
                self.logger.info(f"[MessageDispatcher]: Message received {message}")
    ...
    def start(self):
        ...
        """
        self.logger.info("[MessageDispatcher]: Start")
    ...
    def dispatch_chain(self, chain_id):
        ...
        """
        self.logger.info(f"========DISPATCH CHAIN {chain_id}===========")
    ...
    def dispatch_message(self, message):
        self.logger.info(f"Dispatching Message {message}")
